"""
tests/tpcc_standard/run_experiments.py
Official 9-Table TPC-C Strategy Comparison Framework.
Fully self-contained within tests/tpcc_standard/.

Usage:
------
# Setup database (Scale 2 warehouses)
PYTHONPATH=src python tests/tpcc_standard/setup_db.py --warehouses 2

# Full benchmark run (all CG × CS combinations, 3 iterations)
PYTHONPATH=src python tests/tpcc_standard/run_experiments.py
...
PYTHONPATH=src python tests/tpcc_standard/run_experiments.py --plot-only
"""
...
from tests.tpcc_standard.strategy_runner import run_baseline, run_strategy
from tests.tpcc_standard.measure import prewarm_database_tables
from tests.tpcc_standard.plot import generate_all_plots
from tests.tpcc_standard.tpcc_workload import load_tpcc_queries, fetch_schema
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Official 9-Table TPC-C Strategy Comparison")
...
def get_connection():
    ...
    return psycopg2.connect(
        dbname=os.getenv("TPCC_STANDARD_DB_NAME", "tpcc_standard_db"),
...
def main() -> int:
    ...
    if args.plot_only:
        ...
        print(f"[run_experiments/tpcc_standard] Regenerating plots from: {target_dir}...")
    ...
    print(f"Loaded {len(queries)} official TPC-C queries from queries/")

    schema = fetch_schema(conn)
    print(f"Schema loaded: {len(schema)} tables ({', '.join(sorted(schema.keys()))})\n")

    # Global prewarm: load all 9 tables into memory upfront
    ...
    (run_dir / "summary.txt").write_text(
        f"Workload: TPC-C Standard (9 Tables)\n"
    ...
    with open(run_dir / "summary.json", "w") as f:
        json.dump({
            "workload": "TPC-C Standard (9 Tables)",
            "timestamp": timestamp,
