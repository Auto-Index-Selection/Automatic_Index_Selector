"""
tests/tpcc/plot.py
Visualisation module for TPC-C experiment results.
...
def find_csv_dir(results_dir: Path) -> Path:
    """Find the directory containing result CSVs."""
...
def plot_total_comparison(totals: Dict[str, float], plots_dir: Optional[Path] = None) -> None:
    ...
    ax.set_title("TPC-C: Strategy Comparison — Total Workload Time", fontsize=13, fontweight="bold", pad=12)
...
def plot_per_query_heatmap(per_query: Dict[str, Dict[str, float]], plots_dir: Optional[Path] = None) -> None:
    ...
    ax.set_title("TPC-C: Per-Query Execution Time Heatmap", fontsize=13, fontweight="bold", pad=20)
    plt.tight_layout()
...
def plot_k_curve(totals: Dict[str, float], plots_dir: Optional[Path] = None) -> None:
    ...
    ax.set_title("TPC-C: Index Count vs Workload Time (config_sel)", fontsize=13, fontweight="bold")
    ax.legend()
...
def plot_budget_curves(totals: Dict[str, float], plots_dir: Optional[Path] = None) -> None:
    ...
    ax.set_ylabel("Total Workload Execution Time (seconds)", fontsize=11)
    ax.set_title("TPC-C: Storage Budget vs Workload Time (cs_drop vs cs_extend)", fontsize=13, fontweight="bold")
    ax.legend()
    ax.grid(True, linestyle=":", alpha=0.6)
    plt.tight_layout()
