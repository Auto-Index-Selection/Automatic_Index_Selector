"""
tests/tpcc/measure.py
Measure query execution times for the TPC-C workload across iterations.
...
def measure_workload(
    ...
    -------
    {
        "per_query"   : {label: avg_seconds},
        "total"       : avg_total_seconds,
        "iterations"  : iterations,
        "all_passes"  : {label: [t_it1, t_it2, ...]},
    }
    """
...
def write_csv(
    ...
    Write measurement results to two CSV files:
      1. results_dir / <label>_qt.csv   -> per-query per-iteration timing
      2. results_dir / <label>_avg.csv  -> average time per query
    """
