...
try:
    ...
    numeric_index = numericIndex(candidates=candidateIndexes)
    print(len(numeric_index))


    configs = generateConfigurations(candidate_indexes=candidateIndexes, workload=W[0:20])
    # import json
    # print(json.dumps(configs, indent=4))
    print(len(configs))

    indexSet = numericIndex(candidateIndexes)
    configSet = numericConfig(configs, indexSet)
    # print(configSet)
    conn.close()
